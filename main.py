...
while current_currency not in CURRENCIES:
    print(f"Валюта {current_currency} не поддерживается.")
    current_currency = input("Выберите доступную валюту из списка:")
    
# 4
result_currency = input("Введите результирующую валюту: ")
while result_currency not in CURRENCIES:
    print(f"Валюта {result_currency} не поддерживается.")
    result_currency = input("Выберите доступную валюту из списка:")
# 5
